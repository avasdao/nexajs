...
describe( 'Purse Test Suite', () => {
    ...
    describe( 'Purse -> Get Coins', async () => {
        it( 'should retieve Coins (from UTXOs) of a given WIF', async () => {
            ...
            const wif = encodePrivateKeyWif(hexToBin(PRIVATE_KEY))
...
return
            /* Encode private key WIF. */
            const wif1 = encodePrivateKeyWif(hexToBin(PRIVATE_KEY_1), 'mainnet')
            console.log('PRIVATE KEY (WIF):', wif1)

            /* Encode private key WIF. */
            const wif2 = encodePrivateKeyWif(hexToBin(PRIVATE_KEY_2), 'mainnet')
            console.log('PRIVATE KEY (WIF):', wif2)

            /* Encode private key WIF. */
            const wif3 = encodePrivateKeyWif(hexToBin(PRIVATE_KEY_3), 'mainnet')
            ...
            const publicKey1 = derivePublicKeyCompressed(hexToBin(PRIVATE_KEY_1))
            console.log('PUBLIC KEY 1 (hex)', binToHex(publicKey1))

            /* Derive the corresponding public key. */
            const publicKey2 = derivePublicKeyCompressed(hexToBin(PRIVATE_KEY_2))
            console.log('PUBLIC KEY 2 (hex)', binToHex(publicKey2))

            /* Derive the corresponding public key. */
            const publicKey3 = derivePublicKeyCompressed(hexToBin(PRIVATE_KEY_3))
