...
import { decodePrivateKeyWif } from '../index.js'
...
describe( 'HD Node Test Suite', () => {
    ...
    describe( 'HD Node -> Decode Private Key (WIF)', () => {
        it( 'should encode a private key using WIF', async () => {
            ...
            // Encode Private Key WIF.
            const decoded = decodePrivateKeyWif(sha256, TEST_WIF)
            // console.log('DECODED', decoded)
            // console.log('DECODED (pk):', binToHex(decoded.privateKey))
            expect(binToHex(decoded.privateKey)).to.equal(PRIVATE_KEY) // Nexa (`6` or `??`)
        } )
    } )
    ...
    describe( 'HD Node -> Parse Wallet Import Format (WIF)', () => {
        ...
        it( 'should parse a return address from WIF', async () => {
            ...
            expect(returnAddress).to.equal(P2PKH_TEST_ADDRESS)
        } )
    } )
