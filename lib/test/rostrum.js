...
import { getAddressTokenBalance } from '../index.js'
import { getAddressTokenHistory } from '../index.js'
import { getAddressTokenMempool } from '../index.js'
...
const NEXA_TOKEN_ADDRESS = 'nexa:tqjdvl627lz78s5sr37u65d0rqskla20cpcjytl3n2mxwgsv55qqq09265twm' // DICE
...
describe( 'Rostrum (LIVE) Test Suite', () => {
    ...
    describe( 'Blockchain -> Address -> Balance', () => {
        it( 'should receive an OBJECT (by Class) from the REMOTE server', async () => {
            ...
            console.log('STATUS-1', rostrum.status)
            setTimeout(() => {
                console.log('STATUS-2', rostrum.status)
            }, 1000)
    ...
    describe( 'Token -> Genesis -> Info', () => {
        ...
        it( 'should receive a (NEXA) OBJECT from the REMOTE server', async () => {
            ...
            // console.log('GENESIS INFO (DICE)', result)
            expect(result).to.be.an('object')
        } )
