...
describe( 'Crypto (Unit) Test Suite', () => {
    ...
    describe( 'Crypto -> hash160', () => {
        it( 'should generate a SHA-256 + RIPEMD-160 (HEX) hash of the provided (UTF-8) data', () => {
            const hash = hash160(NEXA_TEST_SECRET)

            expect(hash).to.equal('2259cd5b42ae4d70deaa3d8d2ead2bb32ed3677b')
        } )

    } )

    describe( 'Crypto -> hash256', () => {
        it( 'should generate a double SHA-256 (HEX) hash of the provided (UTF-8) data', () => {
            const hash = hash256(NEXA_TEST_SECRET)

            expect(hash).to.equal('1dbc6e9fc4cee6acc5f3acebaf126827625166d04aaf954ea32f308439ba693d')
        } )
    ...
    describe( 'Crypto -> verifyMessageHashEcdsa', () => {
        it( 'should verify an ECDSA data payload using a address, sig + msg', () => {
            privateKeyBin = sha256(NEXA_TEST_SECRET, 'binary')
    ...
    describe( 'Crypto -> verifyMessageHashSchnorr', () => {
        it( 'should verify a SCHNORR data payload using a sig, msg + pubkey', () => {
    ...
    describe( 'errors', () => {
        it( 'should return a (hashbuf must be a 32 byte buffer) error', () => {
            privateKeyBin = sha256(NEXA_TEST_SECRET, 'binary')

            const signingSerialization = sha256(NEXA_TEST_BODY, 'binary')

            const sighash = ripemd160(sha256(signingSerialization))

            // Generate a signature over the "sighash" using the passed private key.
            const signatureBin = signMessageHashSchnorr(privateKeyBin, sighash)

            expect(signatureBin).to.equal('hashbuf must be a 32 byte buffer')
        } )
        ...
        it( 'should return a (hashbuf must be a 32 byte buffer) error', () => {
            ...
            expect(isValid).to.equal('hashbuf must be a 32 byte buffer')
        } )
    } )
