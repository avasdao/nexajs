...
describe( 'Crypto (Unit) Test Suite', () => {
    ...
    describe( 'Crypto -> decryptForPubkey', () => {
        it( 'should decrypt the data payload using a private key', () => {
            // const privKey = new Uint8Array(32)
            const encrypted = 'SFVTSAMU/exw/rqrZXF+4Yppvs/V2qMupLtxfExbjHnb8azkIJsoJ2evYlQe+sRoNpMjZke30OU2YkKhR68nt73o03DfZmhDyZn7W7VHBTtjDfERmyyqtoICZN94rdkInIZyCUk='

            const privkey = PrivateKey(PRIVATE_KEY)
            ...
            expect(decrypted).to.equal('too-many-secrets')
        } )
    } )
    ...
    describe( 'Crypto -> encrypt', () => {
        it( 'should encrypt the data payload using a private key', () => {
            const msg = 'hi there!'

            // const privKey = new Uint8Array(16)
            const privKey = PRIVATE_KEY.slice(32) // NOTE: Use a 16-bit key for aes-128-cbc.

            const ciphertext = encrypt(privKey, msg)
    ...
    describe( 'Crypto -> encryptForPubkey', () => {
        it( 'should encrypt the data payload for the specified public key', () => {
            // const privKey = new Uint8Array(32)
            const msg = 'too-many-secrets'
console.log('***WTF IS HAPPENING', msg);
            ...
            // expect(ciphertext.toString()).to.equal('U2FsdGVkX18cLqpZCjcp1+haTZ5u61ieUGecJBR0ptg=')
            expect(encrypted.length).to.equal(136)
        } )
    } )
