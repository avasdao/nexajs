...
describe( 'Utils (Unit) Test Suite', () => {
    ...
    describe( 'Utils -> bigIntToBitcoinVarInt', () => {
        it( 'should offer a 64-bit version of a (small) big integer', () => {
            /* Reverse the bytes of the Hex string. */
            const testVal = 1337n

            expect(bigIntToBitcoinVarInt(testVal)).to.eql(new Uint8Array(TEST_ARRAY_16_VARINT))
        } )
    ...
    describe( 'Utils -> numberToBinUintLE', () => {
        // TBD
        const num = 1337
        const uint = numberToBinUintLE(num)
